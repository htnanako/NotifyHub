...
程序支持个人开发者自行开发插件接入主程序，可使用内置的配置项、定时任务、自定义前端页面支持、调用通知渠道/通道等，具体可插件[插件开发指南](https://github.com/htnanako/NotifyHub/blob/main/plugins/%E6%8F%92%E4%BB%B6%E5%BC%80%E5%8F%91%E6%8C%87%E5%8D%97.md)
...
- 本项目需要授权码才能使用。
- 授权码支持7天试用期。
- 获取license_key请联系作者，发送邮件到 `service@nanako.cyou` 提供 `邮箱` 及 `项目名称NotifyHub` ，或者试用TG群组的机器人自助申请。
- 不接受任何理由申请退款，不接受任何理由申请退款，不接受任何理由申请退款。

### 永久授权购买
永久授权价格¥88.00。软件已内置购买授权功能，申请试用码安装容器之后可在 `系统设置` -> `授权信息` 中查看授权信息和购买永久授权。目前仅支持支付宝支付。
